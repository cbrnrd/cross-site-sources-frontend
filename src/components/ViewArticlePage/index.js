...
const ViewArticlePage = () => {
    ...
    const { article, loading } = useSelector(state => state.articles)  // Get article from redux store
    useEffect(() => {
        dispatch(getArticle(id))  // Get article from API
    }, [])
    ...
    return (
        <>
            ...
            <div role="status">
                <svg class="inline w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-red-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {!loading && article.article &&
                (<div className="flex justify-center p-10">
                    <div className="w-1/2">
                        {/* Show image from imageUrl */}
                        <img src={article.article.imageUrl} alt={article.article.title} className="mb-4" />
